...
class BaseValueCachingWrapper(MetaStepMixin, BaseStep, ABC):
    """
    Base class for a value caching wrapper that enables value caching after a fit.
    """
    ...
    def _hash_value(self, data_input: Any) -> str:
        """
        Hash individual data input value.

        :param data_input: data input to hash
        :type data_input: Any

        :return: hashed data input
        """
        m = hashlib.md5()
...
class PickleValueCachingWrapper(BaseValueCachingWrapper):
    """
